...
data NEMem a e = NEMem (Off e) [e] (Frozen a)
...
prop_indexOffMem
  :: forall ma e
   . (Show e, Unbox e, Eq e, MemFreeze ma)
  => NEMem ma e
  -> NonNegative Int
  -> AUnbox
  -> Property
  ...
  conjoin
    ...
    , withAUnbox aPrim $ \e ->
        let tOff = Off k `offForType` e
         in -- test precondition from documentation
            (unOff (toByteOff tOff) <= unCount (byteCountMem fm - byteCount e)) ==> do
              mm <- thawCloneMem fm
...
prop_indexByteOffMem (NEMem off@(Off o) xs fm) (NonNegative k) aPrim =
  conjoin
    ...
    , withAUnbox aPrim $ \a ->
        let tOff = Off k
         in -- test precondition from documentation
            (unOff tOff <= unCount (byteCountMem fm - byteCount a)) ==> do
              mm <- thawCloneMem fm
...
prop_copyAndCompareByteOffToMBytesMem
  :: forall a e
   . (Show e, Unbox e, Eq e, MemFreeze a)
  => NEMem a e
...
prop_copyAndCompareByteOffToMBytesMem (NEMem offSrc xs fm) (NonNegative offByteDst) nn off e =
  propIO $ do
    ...
    when (count > 0) $ do
      ...
      let eOrdering =
            compare
              (singletonBytes (xs !! unOff (offSrc + offDelta)))
...
prop_setMutMem (NEMem off@(Off o) xs fm) e (NonNegative k) =
  propIO $ do
    ...
    emm <- thawMem (emptyMem :: Frozen ma)
    setMutMem emm 0 0 e
...
prop_fromListMemN (NEMem (Off i) xs fm) (Positive n') =
  propIO $ do
    let n = Count $ length xs
        (order', fm' :: Frozen ma) = fromListMemN (Count i) xs
        (order'', fm'' :: Frozen ma) = fromListMemN (n + n') xs
...
prop_loadListMutMem
  :: forall ma e
   . (MemFreeze ma, Show e, Unbox e, Eq e)
  => [e]
...
prop_reallocMutMem (Mem xs fm) (NonNegative n) pt =
  propIO $ do
    mm <- thawCloneMem fm
    withAUnboxType pt $ \aPrimProxy -> do
  ...
  withAUnboxList aPrimList $ \xs ->
    byteCountProxy xs
      ...
      $ do
        ...
        loadListByteOffMutMemN slackCount ss8 mm slackByteOff
          `shouldReturn` (xs8rest, slackCount)
        fm <- freezeMutMem mm
...
memBinarySpec = do
  ...
  describe memTypeName $ do
    ...
    prop "eqMem" $ \(fm1 :: Frozen ma) (fm2 :: Frozen ma) ->
      conjoin
        [ eqMem @Word8 fm1 fm2 === ((convertMem fm1 :: ByteString) == convertMem fm2)
        , fm1 === fm1
        , fm2 === fm2
        ]
