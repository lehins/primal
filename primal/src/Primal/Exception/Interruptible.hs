...
mask action = do
  unsafeIOToPrimal getMaskingState >>= \case
    GHC.Unmasked -> blockAsyncExceptions $ action unblockAsyncExceptions
    GHC.MaskedInterruptible -> action blockAsyncExceptions
...
liftMask action = do
  unsafeIOToPrimal getMaskingState >>= \case
    GHC.Unmasked -> liftBlockAsyncExceptions $ action liftUnblockAsyncExceptions
    GHC.MaskedInterruptible -> liftP (action liftBlockAsyncExceptions)
    GHC.MaskedUninterruptible -> liftP (action liftBlockUninterruptible)
{-# INLINEABLE liftMask #-}
